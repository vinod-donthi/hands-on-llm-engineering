# Week 1 — Start Here

> One-time orientation · then live in [daily/day-01.md](daily/day-01.md)

---

## 1. Setup (once)

```bash
cd week-01
chmod +x scripts/setup-work.sh
./scripts/setup-work.sh
```

Copy `.env.example` → `.env` in your work dir. Never commit `.env`.

---

## 2. Your two folders

| | Where | What you do |
|---|--------|-------------|
| **Learning path** | `week-01/` | Read playbooks, theory, lab specs |
| **Work path** | `week-01-work/` or `~/ai-learning/week-01-work/` | Python, deliverables, project code |

![Learning path markdown repo vs work path with venv and secrets](assets/images/project/start-here-two-paths.svg)

*Figure: Read specs in `week-01/`; run code and store secrets in `week-01-work/` only.*

---

## 3. Every study session

```
Open daily/day-XX.md  →  follow steps 1…N  →  tick progress.md  →  Tomorrow link
```

Do **not** read all theory files before Day 1. The daily playbook links only what you need that day.

---

## 4. First step

**→ [Day 1 playbook](daily/day-01.md)**

Day 7 capstone: follow [project/BUILD-GUIDE.md](project/BUILD-GUIDE.md) from [daily/day-07.md](daily/day-07.md).
